...
You need: - Message framing system\
- Message types\
- Packet format\
- Chunking strategy\
- Error codes\
...
-   FILE_CHUNK { sequence, bytes }\
-   FILE_DONE\
-   ERROR { reason }\
-   Optional: ACK { sequence }
...
## 5. Build the session layer (state machine)
...
## 7. Build the public API

Example:

    FileSender sender("192.168.1.10", 9000);
    sender.send("C:/big.iso");

    FileReceiver receiver(9000);
    receiver.start();
